...
class TgBotConfig {
    companion object {
        ...
        val startMessage = """
            ...
            id банкоматов можно найти на сайте https://www.tinkoff.ru/maps/atm выбрав отдельный банкомат (скриншот ниже). Список необходимо перечислить через запятую, например

            002583,002322,003057

            Изменить список банкоматов можно всегда, просто послав сообщение с новыми id.

            После успешной подписки тебе будут приходить уведомления о пополнении баланса банкоматов, а также если в банкомате стали заканчиваться деньги.
            ...
            Отключить уведомления можно командой: /off
            Возобновить уведомления с текущими id: /on
        """.trimIndent()
    }
                ...
                command("off") {
                    ...
                    try {
                        ...
                        if (subscription == null) {
                            bot.sendMessage(
                                chatId = ChatId.fromId(message.chat.id),
                                text = "Не удалось выключить уведомления, т.к они не были включены"
                            )
                        } else {
